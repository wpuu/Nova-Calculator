...
test('enforces per-key RPM window and resets after one minute', () => {
  let now = 0;
  const pool = new ProviderKeyPool([{ id: 'a', secret: 'secret-a', rpmLimit: 2 }], { now: () => now, paidReserveFraction: 0 });
...
test('free traffic cannot consume paid reserve capacity', () => {
  const pool = new ProviderKeyPool([{ id: 'a', secret: 'secret-a', rpmLimit: 10 }], { paidReserveFraction: 0.2 });
  for (let i = 0; i < 8; i += 1) assert.ok(pool.lease(REQUEST_PRIORITY.FREE));
...
test('snapshot is safe for logs and never exposes raw secrets', () => {
  const pool = new ProviderKeyPool([{ id: 'a', secret: 'do-not-log-me', rpmLimit: 12 }]);
...
test('provider key config accepts comma/newline lists, de-duplicates values and uses Agnes-safe RPM', () => {
  const keys = parseProviderKeys('k1, k2\nk1', 12);
  assert.deepEqual(keys, [
    { id: 'key-1', secret: 'k1', rpmLimit: 12 },
    { id: 'key-2', secret: 'k2', rpmLimit: 12 },
  ]);
  assert.throws(() => parseProviderKeys('k1', 16), /between 1 and 15/);
});
